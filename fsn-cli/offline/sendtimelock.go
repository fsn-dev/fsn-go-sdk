...
var CommandSendTimelock = cli.Command{
	Name:      "sendtimelock",
	Category:  "offline",
	Usage:     "build send timelock raw transaction",
	ArgsUsage: "<assetID> <to> <value>",
	Description: `
build send timelock raw transaction`,
...
func sendtimelock(ctx *cli.Context) error {
	setLogger(ctx)
	if len(ctx.Args()) != 3 {
		cli.ShowCommandHelpAndExit(ctx, "sendtimelock", 1)
	}
