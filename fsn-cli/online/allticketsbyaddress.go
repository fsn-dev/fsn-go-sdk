...
var CommandAllTicketsByAddress = cli.Command{
	...
	ArgsUsage: "<address>",
	Description: `
get all tickets by address`,
	Flags: []cli.Flag{
		blockHeightFlag,
		serverAddrFlag,
	},
	Action: allticketsbyaddress,
}

func allticketsbyaddress(ctx *cli.Context) error {
	setLogger(ctx)
	if len(ctx.Args()) != 1 {
		cli.ShowCommandHelpAndExit(ctx, "allticketsbyaddress", 1)
	}
