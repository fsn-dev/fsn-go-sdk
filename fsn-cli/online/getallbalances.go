...
func getallbalances(ctx *cli.Context) error {
	setLogger(ctx)
	if len(ctx.Args()) != 1 {
		cli.ShowCommandHelpAndExit(ctx, "getallbalances", 1)
	}
	...
	balance, err := client.GetAllBalances(context.Background(), address, blockNr)
	if err != nil {
		return err
	}
